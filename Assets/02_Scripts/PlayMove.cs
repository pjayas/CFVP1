...
public class PlayerMove : MonoBehaviour
{
    ...
    void Start()
    {
        rb = this.GetComponent<Rigidbody2D>(); //rb변수에 실제 사용할 Rigidbody2D 컴포넌트 대입
    ...
    void Update()
    {
        if (BeUsed == false && Input.GetMouseButton(0)) //만약 player가 날고 있지 않은 상태(false)이고 마우스가 눌리지 않았다면
        {
            ...
            if (hit.collider != null && hit.collider.gameObject == this.gameObject) //만약 hit(마우스 포인터 대상)이 아무것도 아닌 상태이거나(충돌된 상태)이고 이 충돌 대상이 this.gameobject 즉 player일 경우에
            {
